    ...
    : mWeights(weights)
{
    ...
    for (int layerIndex = 0; layerIndex < mNumLayers; layerIndex++)
    {
        for (int gateIndex = 0; gateIndex < numGates; gateIndex++)
        {
            ...
            nvinfer1::Weights gateKernelWeights{.type = dataType, .values = (void*) (&mWeights->mWeights[0] + kernelOffset), .count = inputSize * mNumUnits};
            nvinfer1::Weights gateBiasWeights{.type = dataType, .values = (void*) (&mWeights->mWeights[0] + biasOffset), .count = mNumUnits};
